...
def AESEncRound(key, block: list):
	tempMat = subBytes(block)
	tempMat = shiftRows(tempMat) #DONE
	tempMat = mixCols(tempMat) #DONE
	return XOR(key, tempMat) #__AddRoundKey__

def AESEncLastRound(key, block: list):
	tempMat = subBytes(block)
	tempMat = shiftRows(tempMat)
	return XOR(key, tempMat) #__AddRoundKey__

def AESDecRound(key, block: list):
	tempMat = XOR(key, block) #__AddRoundKey__
	tempMat = invMixCols(tempMat) #DONE
	tempMat = invShiftRows(tempMat) #DONE
	tempMat = invSubBytes(tempMat) #DONE
	return tempMat

def AESDecFirstRound(key, block: list):
	tempMat = XOR(key, block) #__AddRoundKey__
...
def mixCol(arr: list):
	tempCol = [0,0,0,0]
	tempCol[0] = GaloisMultiply(2,arr[0]) ^ GaloisMultiply(3,arr[1]) ^ GaloisMultiply(1,arr[2])^ GaloisMultiply(1,arr[3])
	tempCol[1] = GaloisMultiply(1,arr[0]) ^ GaloisMultiply(2,arr[1]) ^ GaloisMultiply(3,arr[2])^ GaloisMultiply(1,arr[3])
	tempCol[2] = GaloisMultiply(1,arr[0]) ^ GaloisMultiply(1,arr[1]) ^ GaloisMultiply(2,arr[2])^ GaloisMultiply(3,arr[3])
	tempCol[3] = GaloisMultiply(3,arr[0]) ^ GaloisMultiply(1,arr[1]) ^ GaloisMultiply(1,arr[2])^ GaloisMultiply(2,arr[3])
	return tempCol
